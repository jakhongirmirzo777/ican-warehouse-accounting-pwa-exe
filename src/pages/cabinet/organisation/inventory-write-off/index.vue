      ...
      <template #item.actions="{ item }">
        <div class="d-flex">
          <VTableActions
            ...
            @view="
              $router.push(
                $localePath(`/cabinet/inventory-write-off-item/${item.id}`)
              )
            "
          />
        </div>
...
import InventoryWriteOffDialog from '@/components/pages/inventory-write-off/InventoryWriteOffDialog.vue'
...
const useFetchWriteOffs = async () => {
  try {
    const {
      data: { data, links },
    } = await fetchWriteOffs(options.value)
